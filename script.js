...
const TWEET_TEMPLATES = [
    { img: "twitter-x.png", text: "🔒 Just sealed my thoughts in a #Bitcoin Time Capsule on Signet! 🚀 Unlocks at block {UNLOCK_BLOCK}. First few unlockers get prizes! Be ready! #BRC20 #Crypto" },
    { img: "twitter-xi.png", text: "Time to send a message to the future via #Bitcoin! ⏳ Using Time Capsule on Signet. Get your wallet ready for block {UNLOCK_BLOCK}. Prizes inside! 😉 #BitcoinTimeCapsule" },
    { img: "twitter-xiii.png", text: "Got a secret for block {UNLOCK_BLOCK}? 🤫 Store it in the #Bitcoin Time Capsule! Running on Signet. Unlockers earn fees! Check it out! 👇 #CryptoNews" },
    { img: "twitter-xiiii.png", text: "Testing out the #Bitcoin Time Capsule on Signet! 🛠️ Write a message, lock it till block {UNLOCK_BLOCK}. Simple, secure, and rewards unlockers! Pretty cool. 😎 #BitcoinDevelopement" }
];
...
function updateWalletDisplay(walletName, address, network, balanceStr = 'Loading...') {
    ...
    if (walletConnected && address) {
        wsEl.style.display = 'none'; wiEl.style.display = 'flex'; waEl.textContent = `${walletName||'Wallet'}: ${shortenAddress(address)}`; wbEl.textContent = `Balance: ${balanceStr}`;
        connBtn.style.display = 'none'; discBtn.style.display = 'inline-flex';
    } else {
        wsEl.style.display = 'block'; wsEl.textContent = 'Wallet: Not Connected'; wiEl.style.display = 'none';
...
async function initWalletConnection() {
    ...
    discBtn.onclick = async () => {
        const name = currentWallet?._brand?.name || "Wallet";
        if (currentWallet && typeof currentWallet.disconnect === 'function') { try { await currentWallet.disconnect(); } catch (e) { console.warn(`Err on ${name}.disconnect: ${e.message}`); }}
        else if (currentWallet && currentWallet._satsConnect && typeof currentWallet._satsConnect.disconnect === 'function') { try { await currentWallet._satsConnect.disconnect(); } catch (e) { console.warn(`Err on SatsConn.disconnect: ${e.message}`); }}
        handleDisconnect(name);
    };
    await checkInitialConnection();
    const tcppLnk = document.getElementById('tcppLink'); if(tcppLnk) tcppLnk.onclick=(e)=>{e.preventDefault();hideModal(true);showModal("Terms & Privacy",`<div class="tcpp-content"><h3>Terms</h3><p>Experimental use on Signet. No real value.</p><h3>Privacy</h3><p>Public data logged. Wallet T&Cs apply.</p></div>`);};
...
const handleAccountsChanged = async (accs, name="Wallet") => {
    if (!accs || accs.length === 0) { handleDisconnect(name); return; } userAddress = accs[0].address || accs[0]; let balStr = 'Loading...';
    if (currentWallet && typeof currentWallet.getBalance === 'function') { try { balStr = formatBalance(await currentWallet.getBalance(), name); } catch (e) { balStr = 'N/A'; }}
    else if (userAddress && currentWallet && currentWallet._satsConnect) { balStr = await fetchSignetBalance(userAddress); }
    if (currentWallet && typeof currentWallet.getNetwork === 'function') { try { const netInfo = await currentWallet.getNetwork(); currentNetwork = (netInfo && typeof netInfo.network === 'string') ? { network: netInfo.network.toLowerCase() } : { network: CONTRACT_CONFIG.network }; } catch (e) { currentNetwork = { network: CONTRACT_CONFIG.network }; }}
    updateWalletDisplay(currentWallet?._brand?.name||name, userAddress, currentNetwork.network, balStr); walletConnected = true; updateEncryptButtonState();
};
const handleNetworkChanged = async (netInfo, name="Wallet") => {
    if (typeof netInfo === 'string') currentNetwork = { network: netInfo.toLowerCase() };
    else if (netInfo && typeof netInfo.network === 'string') currentNetwork = { network: netInfo.network.toLowerCase() };
    else if (currentWallet && typeof currentWallet.getNetwork === 'function') { try { const ni = await currentWallet.getNetwork(); currentNetwork = (ni && typeof ni.network === 'string') ? { network: ni.network.toLowerCase() } : { network: CONTRACT_CONFIG.network }; } catch (e) { currentNetwork = { network: CONTRACT_CONFIG.network }; }}
    else { currentNetwork = { network: CONTRACT_CONFIG.network }; }
    const balStr = document.getElementById('walletBalance')?.textContent.replace('Balance: ','') || 'Loading...';
    updateWalletDisplay(currentWallet?._brand?.name||name, userAddress, currentNetwork.network, balStr);
    if (currentNetwork.network !== CONTRACT_CONFIG.network) await promptNetworkSwitch(currentWallet?._brand?.name||name);
    updateEncryptButtonState();
};
...
async function checkWalletsAvailability() {
    let found = 0; const upd=(key, avail)=>{const opt=document.querySelector(`.wallet-option[data-wallet="${key}"]`); if(opt){opt.classList.toggle('unavailable',!avail);if(avail)found++; opt.style.display='flex';}};
    await new Promise(r=>setTimeout(r,150)); upd('unisat',typeof window.unisat?.requestAccounts==='function');
    const sc = window.satsConnect||window.satsconnect;
    upd('xverse',typeof window.BitcoinProvider==='object'||(typeof sc?.request==='function')); // Simpler check for Xverse
    upd('okx',typeof window.okxwallet?.bitcoin?.connect==='function');
    upd('leather',typeof window.LeatherProvider==='function'||typeof window.Leather==='object'||(typeof sc?.request==='function')); // Simpler check for Leather
    document.getElementById('walletSelectInstruction').style.display='block'; document.getElementById('walletAvailabilityMessage').style.display=found===0?'block':'none';
}
async function connectToWallet(type) {
    let accs=[], name=type.charAt(0).toUpperCase()+type.slice(1), balStr='Loading...'; currentNetwork={network:'unknown'};
    console.log(`Attempting to connect to ${name}`);
    if(type==='unisat' && typeof window.unisat!=='undefined'){accs=await window.unisat.requestAccounts();currentNetwork=await window.unisat.getNetwork();currentWallet=window.unisat;currentWallet._brand={name:"Unisat"};balStr=formatBalance(await currentWallet.getBalance(),"Unisat");}
    else if(type==='okx' && typeof window.okxwallet?.bitcoin!=='undefined'){const res=await window.okxwallet.bitcoin.connect();accs=res.address?[res.address]:(Array.isArray(res)?res:[]);userPublicKey=res.publicKey;currentNetwork=await window.okxwallet.bitcoin.getNetwork();currentWallet=window.okxwallet.bitcoin;currentWallet._brand={name:"OKX"};balStr=formatBalance(await currentWallet.getBalance(),"OKX");}
    else if(type==='xverse'||type==='leather'){
        const sc=window.satsConnect||window.satsconnect; if(!sc||typeof sc.request!=='function')throw new Error(`${name} (SatsConnect) API missing or not initialized.`);
        name=type.charAt(0).toUpperCase()+type.slice(1); console.log(`Using SatsConnect for ${name}`);
        const getAccountsOptions = { purposes: ['payment', 'ordinals'], network: { type: CONTRACT_CONFIG.network === 'signet' ? 'Signet': 'Mainnet' } };
        console.log(`SatsConnect: Requesting accounts with options:`, getAccountsOptions);
        const addrRes=await sc.request('getAccounts', getAccountsOptions); console.log(`SatsConnect: getAccounts response for ${name}:`, addrRes);
        if(addrRes.status==='success'&&addrRes.result.length>0){
            const pAcc=addrRes.result.find(a=>a.purpose==='payment'); if(!pAcc)throw new Error(`${name} no payment address from SatsConnect.`);
            accs=[pAcc.address];userPublicKey=pAcc.publicKey;currentNetwork={network:(pAcc.network?.type||CONTRACT_CONFIG.network).toLowerCase()};balStr=await fetchSignetBalance(pAcc.address);
            currentWallet={_brand:{name},_satsConnect:sc,_paymentAddress:pAcc.address,requestAccounts:async()=>accs,getNetwork:async()=>currentNetwork,getBalance:async()=>fetchSignetBalance(pAcc.address),
                signPsbt:async(psbtB64)=>{console.log(`SatsConnect: Signing PSBT for ${name}`);const sr=await sc.request('signPsbt',{psbtBase64,network:currentNetwork.network.toUpperCase(),signAtIndex:0});if(sr.status==='success')return sr.result.psbtBase64;throw new Error(sr.error?.message||'SatsConnect signPsbt fail.');},
                disconnect:async()=>{if(typeof sc.disconnect==='function')await sc.disconnect();}};
        } else {throw new Error(addrRes.error?.message||`${name} SatsConnect connection failed.`);}
    } else {throw new Error(`${name} provider detection/connection failed.`);}
    if(!accs||accs.length===0)throw new Error("No accounts or permission denied."); userAddress=accs[0].address||accs[0];
    if(!currentNetwork||typeof currentNetwork.network!=='string')currentNetwork={network:CONTRACT_CONFIG.network.toLowerCase()}; else currentNetwork.network=currentNetwork.network.toLowerCase();
    walletConnected=true; updateWalletDisplay(currentWallet._brand?.name||name,userAddress,currentNetwork.network,balStr);
    ...
    if(window.unisat&¤tWallet===window.unisat){window.unisat.removeAllListeners('accountsChanged');window.unisat.removeAllListeners('networkChanged');window.unisat.on('accountsChanged',(a)=>handleAccountsChanged(a,"Unisat"));window.unisat.on('networkChanged',(n)=>handleNetworkChanged(n,"Unisat"));}
    if(window.okxwallet?.bitcoin&¤tWallet===window.okxwallet.bitcoin){window.okxwallet.bitcoin.removeAllListeners('accountsChanged');window.okxwallet.bitcoin.removeAllListeners('networkChanged');window.okxwallet.bitcoin.on('accountsChanged',(r)=>{const a=Array.isArray(r)?r:(r&&r.address?[r.address]:[]);handleAccountsChanged(a,"OKX")});window.okxwallet.bitcoin.on('networkChanged',(n)=>handleNetworkChanged(n,"OKX"));}
}
async function promptNetworkSwitch(name) {
    if(currentNetwork.network.toLowerCase()===CONTRACT_CONFIG.network)return true;
    if(currentWallet&¤tWallet.switchNetwork&&typeof currentWallet.switchNetwork==='function'){try{let targetNetForSwitch=CONTRACT_CONFIG.network; if(name==="Unisat" && CONTRACT_CONFIG.network==="signet") targetNetForSwitch="testnet"; await currentWallet.switchNetwork(targetNetForSwitch);const nNet=await currentWallet.getNetwork();currentNetwork=(!nNet||typeof nNet.network!=='string')?{network:CONTRACT_CONFIG.network}:{network:nNet.network.toLowerCase()};if(currentNetwork.network===CONTRACT_CONFIG.network||(name==="Unisat"&¤tNetwork.network==="testnet"&&CONTRACT_CONFIG.network==="signet")){currentNetwork.network=CONTRACT_CONFIG.network;updateWalletDisplay(name,userAddress,currentNetwork.network,document.getElementById('walletBalance')?.textContent.replace('Balance: ',''));updateEncryptButtonState();return true;}}catch(err){console.warn(`API switchNetwork for ${name} fail:`,err.message);}}
    return new Promise(res=>{showModal("Incorrect Network",`<p>${name} on <strong>${currentNetwork.network.toUpperCase()}</strong>.</p><p>Switch to <strong>${CONTRACT_CONFIG.network.toUpperCase()}</strong>.</p><p>Click "OK" after switching.</p>`,false);const okBtn=document.querySelector('#modalOverlay .modal-footer .btn-primary');if(okBtn)okBtn.onclick=async()=>{hideModal();if(currentWallet&&typeof currentWallet.getNetwork==='function'){try{const nNet=await currentWallet.getNetwork();currentNetwork=(!nNet||typeof nNet.network!=='string')?{network:CONTRACT_CONFIG.network}:{network:nNet.network.toLowerCase()};}catch(e){console.warn("Re-check net err:",e);}} if(name==="Unisat"&¤tNetwork.network==="testnet"&&CONTRACT_CONFIG.network==="signet")currentNetwork.network=CONTRACT_CONFIG.network; updateWalletDisplay(name,userAddress,currentNetwork.network,document.getElementById('walletBalance')?.textContent.replace('Balance: ',''));updateEncryptButtonState();res(currentNetwork.network===CONTRACT_CONFIG.network);};else res(false);});
}
...
function initCarousel() {
    const cont=document.getElementById('twitterCarousel'), prev=document.getElementById('prevSlide'), next=document.getElementById('nextSlide'), indsCont=document.getElementById('carouselIndicators'); if(!cont||!prev||!next||!indsCont)return;
    cont.innerHTML = ''; TWEET_TEMPLATES.forEach(tweet => { const slide = document.createElement('div'); slide.className = 'carousel-slide'; slide.innerHTML = `<div class="carousel-slide-content"><img src="${tweet.img}" alt="Tweet preview" class="carousel-slide-image"><p class="carousel-slide-text">${tweet.text.replace('{UNLOCK_BLOCK}', CONTRACT_CONFIG.unlockBlockHeight).replace(APP_URL, `<a href='${APP_URL}' target='_blank'>${APP_URL}</a>`)}</p><div class="carousel-slide-actions"><button class="copy-tweet-btn" data-tweet="${tweet.text.replace('{UNLOCK_BLOCK}', CONTRACT_CONFIG.unlockBlockHeight)} ${APP_URL}">Tweet This</button></div></div>`; cont.appendChild(slide); });
    const slides=cont.querySelectorAll('.carousel-slide'), total=slides.length; if(total===0)return;
    let curr=0; indsCont.innerHTML=''; slides.forEach((s,i)=>{s.classList.remove('active');const ind=document.createElement('div');ind.className='carousel-indicator';ind.dataset.slide=i;indsCont.appendChild(ind);});
    const inds=indsCont.querySelectorAll('.carousel-indicator'); function go(idx){curr=(idx+total)%total;slides.forEach((s,i)=>s.classList.toggle('active',i===curr));inds.forEach((ind,i)=>ind.classList.toggle('active',i===curr));} function auto(){go(curr+1);}
    if(total>1){prev.style.display='flex';next.style.display='flex';indsCont.style.display='flex';prev.onclick=()=>{go(curr-1);resetAuto();};next.onclick=()=>{go(curr+1);resetAuto();};inds.forEach((ind,i)=>ind.onclick=()=>{go(i);resetAuto();});if(carouselAutoSlideInterval)clearInterval(carouselAutoSlideInterval);carouselAutoSlideInterval=setInterval(auto,7000);}else{prev.style.display='none';next.style.display='none';indsCont.style.display='none';} function resetAuto(){if(total>1){clearInterval(carouselAutoSlideInterval);carouselAutoSlideInterval=setInterval(auto,7000);}}
...
function updateEncryptButtonState() {
    ...
    if(!walletConnected){btn.disabled=true;btn.textContent="Connect Wallet First";btn.classList.add('btn-visually-disabled'); return;}
    if(currentNetwork.network.toLowerCase()!==CONTRACT_CONFIG.network){btn.disabled=true;btn.textContent=`Use ${CONTRACT_CONFIG.network.toUpperCase()} Network`;btn.classList.add('btn-visually-disabled'); return;}
    btn.classList.remove('btn-visually-disabled');
    if(trimTxt===''){btn.disabled=true;btn.textContent="Enter Message to Proceed";}
    else if(chars>150){btn.disabled=true;btn.textContent="Message Too Long (150 Chars)";}
    else if(bytes>80){btn.disabled=true;btn.textContent="Message Too Large (80 Bytes)";}
...
function checkMessage(){const txIn=document.getElementById('txIdInput'),statDv=document.getElementById('messageStatus'),txId=txIn.value.trim();if(!txId){statDv.innerHTML='<p class="alert alert-warning">Enter TXID.</p>';return;} statDv.innerHTML=`<p>Checking ${txId} (demo)...</p>`;setTimeout(()=>{const unl=Math.random()>0.5,dEnc='SGVsbG8gdGhlcmUh';statDv.innerHTML=`<div class="message-item alert ${unl?'alert-success':'alert-info'}"><p><strong>TXID:</strong> ${txId}</p><p><strong>Status:</strong> ${unl?'Unlocked':'Pending'} (Demo)</p>${unl?`<p class="code-block small">${dEnc}</p><button class="btn btn-primary btn-sm mt-sm" onclick="decodeAndDisplayMessage('${dEnc}')">Decode</button>`:''}</div>`;},500);}
...
function updateVisitorCounter(){const el=document.getElementById('visitorCount');if(el){let c=parseInt(localStorage.getItem('visitor_tc_v6')||'0');c++;el.textContent=c.toLocaleString();localStorage.setItem('visitor_tc_v6',c.toString());}}
